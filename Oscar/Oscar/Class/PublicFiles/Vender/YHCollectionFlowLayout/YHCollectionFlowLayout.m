...
-(instancetype)init {
    self = [super init];
    if (self) {
        self.itemSize = CGSizeMake(kScreenWidth, CELL_HEIGHT);
...
-(CGSize)collectionViewContentSize {
    return CGSizeMake(kScreenWidth, HEADER_HEIGHT+DRAG_INTERVAL*self.count+([UIScreen mainScreen].bounds.size.height-DRAG_INTERVAL));
}
...
-(NSArray*)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    if(current_floor == 0 || current_floor == 1){ //因为导航栏和当前CELL的高度特殊，所有做特殊处理
        correctRect = CGRectMake(0, 0, kScreenWidth, RECT_RANGE);
    }else{
        correctRect = CGRectMake(0, HEADER_HEIGHT+HEADER_HEIGHT+CELL_HEIGHT*(current_floor-2), kScreenWidth, RECT_RANGE);
    }
    ...
    if(screen_y >= HEADER_HEIGHT){
        for(UICollectionViewLayoutAttributes *attributes in array){
            ...
            if(row < current_floor){
                attributes.zIndex = 7;
                attributes.frame = CGRectMake(0, (HEADER_HEIGHT-DRAG_INTERVAL)+DRAG_INTERVAL*row, kScreenWidth, CELL_CURRHEIGHT);
                ...
                attributes.zIndex = 9;
                attributes.frame = CGRectMake(0, attributes.frame.origin.y+(current_floor-1)*offsetOfNextItem-riseOfCurrentItem*percent, kScreenWidth, CELL_HEIGHT+(CELL_CURRHEIGHT-CELL_HEIGHT)*percent);
                ...
                attributes.frame = CGRectMake(0, attributes.frame.origin.y+(current_floor-1)*offsetOfNextItem+offsetOfNextItem*percent, kScreenWidth, CELL_HEIGHT);
                [self setEffectViewAlpha:0 forIndexPath:attributes.indexPath];
            }
            
            [self setImageViewOfItem:(screen_y-attributes.frame.origin.y)/kScreenHeight*IMAGEVIEW_MOVE_DISTANCE withIndexPath:attributes.indexPath];
...
-(void)setImageViewOfItem:(CGFloat)distance withIndexPath:(NSIndexPath *)indexpath
{
    ...
    cell.imageView.frame = CGRectMake(0, IMAGEVIEW_ORIGIN_Y+distance, kScreenWidth, cell.imageView.frame.size.height);
}
